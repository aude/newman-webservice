...
func getQueryParameter(_url *url.URL, name string) (value string, err error) {
	...
	if _, ok := query[name]; !ok {
		return "", fmt.Errorf("query parameter missing: environment")
	}
	if len(query[name]) > 1 {
		return "", fmt.Errorf("more than one query parameter with the same name: environment")
	}
...
func collectionHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "so, you wanna access %v, %v?\n", collectionID, environmentID)
}
